...
def generate_patched_saliency(videos_frames_list, i):
	# create a directory for patched frames for each video saliency map
	p_video = '{0:03d}'.format(i)
	patched_path = '/data1/mohamed/patched_saliency/'+p_video+'/'
	saliency_path = '/data1/mohamed/saliency/'+p_video+'/'
	...
	for view in video_all_viewers:
	  ...
	  for frame in range(4, len(view[1])-5):
		...
		scipy.misc.imsave(viewer_path+"/"+frame_name+".jpg", get_patched_frame(saliency_path+frame_name+'.jpg',
												  sampled_frames[0][-1]))
	  j+=1

def generate_full_saliency_patches(train_ids, test_ids):
	os.system('mkdir /data1/mohamed/saliency')
	# create patched saliency maps for frames in videos of training
	Parallel(n_jobs=20)(delayed(generate_patched_saliency)(videos_frames_list, i) for i in train_ids)
	# create patched saliency maps for frames in videos of testing
	Parallel(n_jobs=20)(delayed(generate_patched_saliency)(videos_frames_list, i) for i in test_ids)
  
generate_full_saliency_patches(train_ids, test_ids)