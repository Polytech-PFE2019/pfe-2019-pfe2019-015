...
def masked_frame(frame, gaze_pos):
  ...
  result = cupy.multiply(gaussian_mask, frame)
...
def generate_gaussian_fov(videos_frames_list, i):
	...
	gaussian_path = '/data1/mohamed/gaussian/'+p_video+'/'
	video_path = '/data1/mohamed/Videos/'+p_video+'/'
	...
	for view in video_all_viewers:
	  ...
	  for frame in range(4, len(view[1])-5):
		...
		scipy.misc.imsave(viewer_path+"/"+frame_name+".jpg", cupy.asnumpy(masked_frame(cupy.array(plt.imread(video_path+frame_name+'.jpg')),
												  sampled_frames[0][-1])))
	  j+=1
 
def generate_full_gaussian_fov(train_ids, test_ids):
	# create gaussian masks for frames in videos of training
	Parallel(n_jobs=20)(delayed(generate_gaussian_fov)(videos_frames_list, i) for i in train_ids)
	# create gaussian masks for frames in videos of testing
